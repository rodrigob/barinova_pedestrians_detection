...
CHoughForest::~CHoughForest()
{
  ...
  for (int i = 0; i < m_vecResizedMultiImages.size(); i ++)
  {
...
// =======================================================================================================
void CHoughForest::SetNumberOfTrees(int in_iNumberOfTrees)
{
                                                    ...
                                                    std::vector <float> &out_BgProbsOfScales)
{
  ...
  for (int idxScale = 0; idxScale < m_iNumberOfScales; idxScale ++)
  {
    ...
    HoughSample hs; 
    hs.left = __min(__max(0, scaledPatchLeftX), curWidth - patchSize-1);
    hs.top = __min(__max(0, scaledPatchTopY), curHeight - patchSize-1);
                ...
                int in_iFirstCol, int in_iLastCol)
{
  ...
  int firstScaledRow = __max(0, in_iFirstRow - in_iBlurRadius);
  int firstScaledCol = __max(0, in_iFirstCol - in_iBlurRadius);
  int lastScaledRow = __min(curHeight-1, in_iLastRow + in_iBlurRadius);
  int lastScaledCol = __min(curWidth-1, in_iLastCol + in_iBlurRadius);
...
{  
  ...
  int firstScaledRow = __max(0, in_uiScaledPatchCenterY - HalfBBoxHeight);
  int firstScaledCol = __max(0, in_uiScaledPatchCenterX - HalfBBoxWidth);
  int lastScaledRow = __min(curHeight-1, in_uiScaledPatchCenterY + HalfBBoxHeight);
  int lastScaledCol = __min(curWidth-1, in_uiScaledPatchCenterX + HalfBBoxWidth);
  ...
  {  
    ...
    if (leaf_bg_prob < m_fBgThresh)
    {
      ...
      out_iFirstCol = __min(out_iFirstCol, instanceX);
      out_iLastCol = __max(out_iLastCol, instanceX);
      out_iFirstRow = __min(out_iFirstRow, instanceY);
      out_iLastRow = __max(out_iLastRow, instanceY);
        }
      }
    }
  }
  ...
  out_iFirstCol = __max(firstScaledCol, out_iFirstCol);
  out_iLastCol = __min(lastScaledCol, out_iLastCol);
  out_iFirstRow = __max(firstScaledRow, out_iFirstRow);
  out_iLastRow = __min(lastScaledRow, out_iLastRow);
}
